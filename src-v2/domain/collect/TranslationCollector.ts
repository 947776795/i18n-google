...
import { JSXTextExtractor } from "./JSXTextExtractor";
...
export class TranslationCollector {
  ...
  constructor(private config: I18nConfig) {
    ...
    this.stringExtractor = new StringExtractor(config);
  ...
  async processFile(filePath: string): Promise<FileResult> {
    ...
    try {
      ...
      const extractions: ExtractedContent[] = [
        ...this.stringExtractor.extract(source, filePath),
        ...this.templateExtractor.extract(source, filePath),
        ...this.jsxTextExtractor.extract(source, filePath),
      ];
